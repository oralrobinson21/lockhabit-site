...
export const captureCreatorReferral=createServerFn({method:"POST"}).validator((d:{slug:string;landingPath:string})=>inputSchema.parse(d)).handler(async({data})=>{
 ...
 const {data:rows,error}=await supabaseAdmin.rpc("capture_creator_referral",{p_slug:data.slug,p_landing_path:data.landingPath,p_referrer_host:undefined,p_user_agent_hash:undefined});
