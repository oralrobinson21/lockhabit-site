...
function SetCreatorPassword(){
 ...
 useEffect(()=>{let active=true; const verify=async()=>{const {data}=await supabase.auth.getSession(); if(!data.session){if(active)setReady(false);return;} const {data:profile}=await supabase.from("creator_profiles").select("id,status").eq("auth_user_id",data.session.user.id).in("status",["approved","active"]).maybeSingle(); if(active)setReady(Boolean(profile));}; void verify(); const {data}=supabase.auth.onAuthStateChange(()=>{void verify();}); return()=>{active=false;data.subscription.unsubscribe();};},[]);
