      ...
      <section className="relative overflow-hidden border-b-2 border-foreground bg-[#f5e5bd] px-5 py-16 lg:px-10 lg:py-24">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_.8fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-4 font-slab text-[clamp(3.4rem,9vw,7.4rem)] uppercase leading-[.86]"><span className="text-coral">Stories are</span><br /><span className="text-[#173c2d]">checking in.</span></h1>
            <p className="mt-6 max-w-xl font-display text-xl leading-relaxed">We’re building a place for ingredient stories, research notes, routines, source links, and the occasional sunny detour.</p>
            <p className="mt-4 max-w-xl text-muted-foreground">This route stays hidden/noindexed until real articles are ready. No thin content gets pushed into search just to fill a page.</p>
            <div className="mt-7 flex flex-wrap gap-3">
              <Link to="/journal" className="primary-button"><ArrowLeft size={16}/> Back to Journal handoff</Link>
              <Link to="/journal/article-template" className="secondary-button bg-background"><BookOpen size={16}/> View article template</Link>
            </div>
          </div>
          <div className="relative min-h-[420px]">
            <div className="absolute left-[8%] top-[8%] w-[78%] rotate-[-3deg] rounded-[1.5rem] border-2 border-foreground bg-paper p-7 shadow-[8px_8px_0_var(--color-foreground)]">
              <div className="flex items-center justify-between border-b border-foreground/20 pb-4">
                <img src={logoTransparent} alt="LOCKHABIT" className="h-14 w-36 object-contain object-left mix-blend-multiply" />
                <Sparkles className="text-coral" />
              </div>
              <p className="memo mt-5 text-primary">Front desk note</p>
              <h2 className="mt-3 font-display text-4xl font-semibold">Journal entries on the horizon.</h2>
              <p className="mt-4 text-muted-foreground">Real research. Real references. Mixed results included when they exist. No miracle copy.</p>
              <div className="mt-6 flex items-center gap-3 rounded-full border-2 border-foreground bg-muted px-4 py-3">
                <Search size={18}/><span className="text-sm text-muted-foreground">Search appears when articles arrive</span>
              </div>
